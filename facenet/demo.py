...
model = FaceNetModel(config.emd_size, config.class_nums).to(device)
...
def main():
    ...
    # #判断人脸库是否有样本
    # if not len(glob.glob("dataset/test/*")):
    #     #人脸录入
    #     ret, img = cap.read()
    #     #人脸矫正
    ...
    image_files = glob.glob("C:\\Users\\User\\PycharmProjects\\face_detect\\dataset\\CASIA_data\\*.jpg")

    old_img_files = glob.glob("C:\\Users\\User\\PycharmProjects\\face_detect\\dataset\\test\\*.jpg")

    print(f"发现 {len(image_files)} 张测试图像")
    print(f"发现 {len(old_img_files)} 张CASIA图像")
    for f in image_files:
        print(f"path type: {type(f)}, path content: {f}")

        img1 = f
        # cv2.imshow("face", face_cor(img1))
        if img1 is None:
            print(f"无法读取图像 {f}")
            continue

        for n in old_img_files:
            img2 = n
            if img2 is None:
                print(f"无法读取图像 {n}")
                continue

            # 假设 face_net 接收两个图像数组并返回距离
            d = face_net(img1, img2)

            if d < s_d:
                print(f"✅ 匹配图像: {n}, <UNK> <UNK>: {d}")
            else:
                print(f"❌ 失败: {n}")
